...
const NATIONAL_MEDICAL_URL="./kokkoritsu-medical.html";
...
const data=[
{name:'国際医療福祉大学',place:'千葉県成田市',pref:'千葉県',total:1850,first:null,badge:'低学費',status:'一部確認中',tags:['分割納入あり','特待制度あり','成田キャンパス'],tip:'6年間総額は低い水準。初年度内訳と医学部固有の別途費用は募集要項で最終確認が必要。',special:'特待奨学生制度あり',installment:'分割・二段階納入制度あり',extra:'教育後援会費等を確認',url:'https://admissions.iuhw.ac.jp/expenses/'},
...
{name:'藤田医科大学',place:'愛知県豊明市',pref:'愛知県',total:2152,first:null,badge:'2026値下げ',status:'総額確認済み',tags:['2026年度から値下げ','学援ローン','東海'],tip:'2026年度入学生から6年間総額2,152万円へ大幅値下げ。初年度の詳細内訳は募集要項で確認。',special:'独自ローン・奨学制度あり',installment:'FUJITA学援ローンあり',extra:'諸会費等は募集要項で確認',url:'https://www.fujita-hu.ac.jp/news/vsfo8q0000002die.html'},
...
{name:'東邦大学',place:'東京都大田区',pref:'東京都',total:2580,first:null,badge:'特待制度',status:'一部確認中',tags:['初年度授業料免除','在学生特待','東京'],tip:'一般入試成績優秀者への初年度授業料免除制度がある。学納金総額の最新年度確認を継続中。',special:'初年度授業料全額または半額免除等',installment:'公式ページで要確認',extra:'委託徴収金等を確認中',url:'https://www.toho-u.ac.jp/info_exam/fee_nyusi.html'},
{name:'昭和医科大学',place:'東京都品川区ほか',pref:'東京都',total:2700,first:null,badge:'全寮制',status:'一部確認中',tags:['初年度全寮制','前後期2回払い','延納制度'],tip:'1年次は富士吉田キャンパスで全寮制。寮費を学費とは別に必ず見込む必要がある。',special:'特待・奨学制度あり',installment:'前後期2回払い・延納制度あり',extra:'1年次寮費等が別途',url:'https://adm.showa-u.ac.jp/admission/expenses/'},
{name:'東京医科大学',place:'東京都新宿区',pref:'東京都',total:2940,first:null,badge:'都心',status:'一部確認中',tags:['奨学金','地域枠','新宿'],tip:'学納金に加え、同窓会費などの委託徴収金が設定される場合があるため要項確認が必要。',special:'奨学金・地域枠制度あり',installment:'公式ページで要確認',extra:'委託徴収金等を確認中',url:'https://www.tokyo-med.ac.jp/med/payment.html'},
...
{name:'東京女子医科大学',place:'東京都新宿区',pref:'東京都',total:null,first:null,badge:'2027値下げ予定',status:'調査中',tags:['女子大学','東京','2027学納金改定'],tip:'2027年度から医学部の入学金を200万円→160万円、授業料を年280万円→250万円へ減額すると大学が公表。2026年度総額は募集要項で最終確認中。',special:'2027年度以降の学納金減額を公式発表',installment:'入学手続・納付条件は募集要項で確認',extra:'2027年度は現行から6年間計220万円の負担軽減予定',url:'https://www.twmu.ac.jp/univ/news/detail.php?cd=1754&kbn=1&ym=202606'},
...
function yen(n){return n==null?'確認中':n.toLocaleString()+'万円'}
...
function getFiltered(){
  ...
  let arr=data.filter(x=>{
    ...
    const statusOk=status==='all'||(status==='priced'&&x.total!=null)||(status==='verified'&&x.status.includes('公式'))||(status==='pending'&&x.total==null&&!x.status.includes('公式確認済み'));
...
function render(){
  ...
  document.querySelector('#cards').innerHTML=shown.map((x,i)=>`<article class="card ${x.total==null&&!x.status.includes('公式確認済み')?'pendingcard':''}"><div class="topline"><span><span class="badge">${x.badge}</span> <span class="tag">${x.status}</span></span><span class="rank">${x.total==null?'確認中':'#'+(i+1)}</span></div><h3>${x.name}</h3><div class="place">${x.place}</div><div class="primarymoney ${x.total==null?'pendingmoney':''}"><span>6年間学費</span><b>${yen(x.total)}</b><small>大学公表額ベース</small></div><div class="minimoney"><div class="${x.first==null?'pendingmoney':''}"><span>初年度</span><b>${yen(x.first)}</b></div><div><span>生活費 初期値</span><b>月${defaultLife(x)}万円</b></div><div class="${x.total==null?'pendingmoney':''}"><span>生活費込み</span><b>${yen(burden(x))}</b></div></div><div class="tags">${x.tags.slice(0,3).map(t=>`<span class="tag">${t}</span>`).join('')}</div><details class="tipdetails"><summary>先に知っておきたい</summary><div class="tipbody">${x.tip}</div></details><div class="actions">${x.url?`<a class="btn primary" target="_blank" rel="noopener" href="${x.url}">公式で確認</a>`:`<span class="btn" aria-disabled="true">公式資料を調査中</span>`}<button class="btn ${selected.includes(x.name)?'selected':''}" ${x.total==null?'disabled title="学費確認後に比較できます"':''} data-compare="${escapeHtml(x.name)}">${x.total==null?'比較準備中':selected.includes(x.name)?'比較から外す':'比較に追加'}</button></div></article>`).join('')||'<p>条件に合う大学がありません。</p>';
}
